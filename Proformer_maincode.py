...
def find_common_healthy_controls(disease_label_files, position_path):
    ...
    # Initialize with all valid eids
    common_healthy_eids = valid_eids.copy()

    # For each disease, find healthy controls and intersect
    for label_file in disease_label_files:
        disease_name = os.path.basename(label_file).split('_labels.csv')[0]
        labels_df = pd.read_csv(label_file)

        # Get healthy controls for this disease
        healthy_eids = set(labels_df[labels_df['label'] == 0]['eid'].unique())

        # Intersect with common healthy controls
        common_healthy_eids = common_healthy_eids.intersection(healthy_eids)

        logging.info(f"Disease {disease_name}: {len(healthy_eids)} healthy controls, "
                     f"common healthy: {len(common_healthy_eids)}")
...
def load_and_prepare_data(features_path, labels_path, position_path, protein_corr_path,
                          protein_seq_path, common_healthy_eids=None):
    ...
    # If common_healthy_eids is provided, use only those for controls
    if common_healthy_eids is not None:
        # Get all cases (label=1) for this disease
        cases_df = labels_df[labels_df['label'] == 1]

        # Get common healthy controls
        healthy_df = labels_df[labels_df['eid'].isin(common_healthy_eids)]
        healthy_df = healthy_df[healthy_df['label'] == 0]  # Double check they are healthy

        # Combine cases and common healthy controls
    ...
    # In expression data, all columns except 'eid' and 'label' are proteins.
    ...
    # Standardize names in bias matrix and sequence features
    ...
    # Adjust protein sequence features order
...
# -------------------------------
# Data padding and splitting functions
# -------------------------------
...
# -------------------------------
# Modified training and evaluation functions to calculate all metrics
# -------------------------------
...
# -------------------------------
def main():
    features_path = 'imputed_data.csv'
    position_path = 'position_danbai.csv'
    ...
    for label_file in disease_files:
        ...
        # Load data with common healthy controls
        ...
        for epoch in range(epochs):
            ...
            logging.info(f"Train Loss: {train_loss:.4f}, AUC: {train_auc:.4f}, Acc: {train_acc:.4f}, "
                         f"Prec: {train_prec:.4f}, Rec: {train_rec:.4f}, F1: {train_f1:.4f}")
            logging.info(f"Val Loss: {val_loss:.4f}, AUC: {val_auc:.4f}, Acc: {val_acc:.4f}, "
        ...
        # Use best model for full dataset prediction
    ...
    # Save comprehensive summary of all diseases
