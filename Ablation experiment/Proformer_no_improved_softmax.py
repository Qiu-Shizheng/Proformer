...
# Device configuration, using GPU0 as the main device
...
# 2. Define Model-Related Classes
...
class SegmentTransformer(nn.Module):
    def __init__(self, segment_size, d_token, n_heads, n_layers, dim_forward, dropout, bias_matrix):
        ...
        assert bias_tensor.shape == (segment_size, segment_size), "Incorrect bias matrix dimensions"
...
class CustomMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, need_weights=True, attn_mask=None):
        # Combine existing attn_mask with bias_matrix if both are provided
        if self.bias_matrix is not None:
            if attn_mask is not None:
                # Ensure both masks are of the same shape
                combined_attn_mask = attn_mask + self.bias_matrix
            else:
                combined_attn_mask = self.bias_matrix
        else:
...
def optimize_hyperparameters(train_dataset, val_dataset, segment_size, num_segments):
    def objective(trial):
        ...
        if available_gpu_count >= 4:
            ...
            print("Less than 4 GPUs available, using a single GPU for training.")

        criterion = nn.BCELoss()
        optimizer = optim.Adam(model.parameters(), lr=lr)

        # Training epochs (adjust as necessary)
        epochs = 5
        best_auc = 0
        ...
        for epoch in range(epochs):
            ...
            if val_auc > best_auc:
                best_auc = val_auc
                # Save best model
                torch.save(model.state_dict(), f'best_model_tmp_no_improved_softmax_{disease}.pth')
...
# 5. Main Loop: Process each disease
for disease in diseases:
    ...
    if not os.path.exists(labels_path):
        print(f"Warning: Label file {labels_path} does not exist. Skipping this disease.")
        continue
    ...
    # Get feature names and bias matrix protein names
    feature_names = X_df.columns.tolist()
    bias_feature_names = bias_df.index.tolist()

    # Get intersection of feature names and bias matrix protein names
    ...
    # Check if the number of features matches the dimensions of the bias matrix
    assert bias_matrix.shape[0] == X.shape[1], "Mismatch between bias matrix dimensions and number of features. Please check."
    ...
    segment_size = 512  # Each segment has 512 features
    num_segments = int(np.ceil(num_features / segment_size))
    print(f"Features are divided into {num_segments} segments, each segment length is {segment_size}.")

    # Update bias_matrix by segmenting and pad the last segment if necessary
    bias_matrices = []
    for i in range(num_segments):
    ...
    X_padded = pad_features(X, segment_size)  # Full dataset also needs to be padded
    ...
    best_params = optimize_hyperparameters(train_dataset, val_dataset, segment_size, num_segments)
    ...
    # Use GPUs (GPU0, GPU1, GPU2, and GPU3) if available
    if available_gpu_count >= 4:
        model = nn.DataParallel(model, device_ids=[0, 1, 2, 3], output_device=0)
        print(f"Using {len([0, 1, 2, 3])} GPUs for training: GPU0, GPU1, GPU2, and GPU3")
    else:
    ...
    for epoch in range(epochs):
        ...
        if val_auc > best_auc:
            best_auc = val_auc
            torch.save(model.state_dict(), f'best_final_model_no_improved_softmax_{disease}.pth')
            print(f"Full model saved as 'best_final_model_no_improved_softmax_{disease}.pth'.")

    # 6. Plot ROC Curve and Report Metrics
    # Load the best saved model
    model.load_state_dict(torch.load(f'best_final_model_no_improved_softmax_{disease}.pth', map_location=device))
    model.eval()
    ...
    # Save prediction probabilities for all samples
    result_df = pd.DataFrame({
        'eid': all_eids,
        'probability': all_preds,
        'label': all_targets
    })
    result_df.to_csv(f'risk_probabilities_no_improved_softmax_{disease}.csv', index=False)
    print(f"Risk probabilities for all samples saved to 'risk_probabilities_no_improved_softmax_{disease}.csv'.")
    ...
    plt.title(f'ROC_no_improved_softmax_{disease}')
    plt.legend(loc="lower right")
    plt.savefig(f'roc_curve_no_improved_softmax_{disease}.pdf')
    plt.close()
    print(f"ROC curve saved as 'roc_curve_no_improved_softmax_{disease}.pdf'.")

    print(f"======== Completed disease: {disease} ========\n")